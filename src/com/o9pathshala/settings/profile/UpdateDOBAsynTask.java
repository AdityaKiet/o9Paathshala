...
public class UpdateDOBAsynTask extends AsyncTask<String, String, Void>{
	...
	private String ip, dob;
	/*private SessionDTO sessionDTO;*/
	private SessionDTO sessionDTO;
	private ProfileDTO profileDTO;
	public UpdateDOBAsynTask(final Context context, String dob) {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				((Activity)context).runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						if(null != context && UpdateDOBAsynTask.this.isCancelled())
	...
	protected void onPreExecute() {
		...
		progressDialog.setOnCancelListener(new OnCancelListener() {
			...
			public void onCancel(DialogInterface arg0) {
				UpdateDOBAsynTask.this.cancel(true);
			}
		});
		super.onPreExecute();
	}
	...
	protected Void doInBackground(String... params) {
		...
		list.add(new BasicNameValuePair("id", String.valueOf(sessionDTO.getId())));
		list.add(new BasicNameValuePair("institue_id", sessionDTO.getCurrentInstitutesId()+""));
		list.add(new BasicNameValuePair("dob", dob));
		try {
			HttpClient httpClient = new DefaultHttpClient();
			HttpPost httpPost = new HttpPost(ip+"/o9pathshala/update_dob.php");
	...
	protected void onPostExecute(Void v) {
		...
		try{
			if(Integer.parseInt(result) > 0){
				Toast.makeText(context.getApplicationContext().getApplicationContext(),"Date of Birth has been updated", Toast.LENGTH_LONG).show();
				profileDTO.setDob(dob);
				UpdateSession.update(context, profileDTO, sessionDTO);
			}else{
			}
		}catch(Exception e){
			Toast.makeText(context.getApplicationContext().getApplicationContext(),"Exception " + e.getMessage(), Toast.LENGTH_LONG).show();
