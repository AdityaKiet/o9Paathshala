...
public class UpdateGenderAsynTask extends AsyncTask<String, String, Void>{
	...
	public UpdateGenderAsynTask(final Context context, Character gender) {
		...
		TimerTask task = new TimerTask() {
			...
			public void run() {
				...
				((Activity)context).runOnUiThread(new Runnable() {
					...
					@Override
					public void run() {
						if(null != context && UpdateGenderAsynTask.this.isCancelled())
	...
	protected void onPreExecute() {
		...
		progressDialog.setOnCancelListener(new OnCancelListener() {
			...
			public void onCancel(DialogInterface arg0) {
				UpdateGenderAsynTask.this.cancel(true);
			}
		});
		super.onPreExecute();
	}
	...
	protected Void doInBackground(String... params) {
		...
		try {
			...
			HttpPost httpPost = new HttpPost(ip+"/o9pathshala/update_gender.php");
	...
	protected void onPostExecute(Void v) {
		...
		try{
			if(Integer.parseInt(result) > 0){
				Toast.makeText(context.getApplicationContext().getApplicationContext(),"Gender has been updated", Toast.LENGTH_LONG).show();
				profileDTO.setGender(gender);
				UpdateSession.update(context, profileDTO, newSessionDTO);
			}else{
				Toast.makeText(context.getApplicationContext().getApplicationContext(),"Gender not updated", Toast.LENGTH_LONG).show();
			}
		}catch(Exception e){
			Toast.makeText(context.getApplicationContext().getApplicationContext(),"Exception " + e.getMessage(), Toast.LENGTH_LONG).show();
		}
